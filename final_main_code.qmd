...
## casewhen filter for DC/ Baltimore to be included only under the proper conditions
data <- data %>%
  group_by(STATEFIP) %>%
...
# Making our recipe
tot_rec <- 
  recipe(a_FMR_fac ~ HHINCOME + PUMA + KITCHEN + BUILTYR2 + TRANTIME + POVERTY + pop2017, data = join_tot) %>%
  step_dummy(PUMA) %>%
...
# Making our recipe
DC_rec <- 
  recipe(a_FMR_fac ~ HHINCOME + PUMA + KITCHEN + BUILTYR2 + TRANTIME + POVERTY + pop2017, data = join_dc) %>%
  step_dummy(PUMA) %>%
  step_zv(all_predictors()) %>%
  step_normalize(all_predictors()) %>%
  step_corr(all_predictors())
...
\*\*KB geography coefficent can capture the issues of measurement between the two places. But combining them together is kb: use caseweights https://www.tidyverse.org/blog/2022/05/case-weights/
