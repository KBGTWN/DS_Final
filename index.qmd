...
A logit-LASSO model, utilizing the lambda hyperparameter (which will be tuned for), as well as two important steps in its recipe:
1. step_dummy() to turn the PUMA codes into dummy variables to consider individually across the dataset
2. step_normalize(all_predictors()) to ensure our predictors are spread on a normal distribution for the LASSO model to interpret and subsequently reduce appropriately.
